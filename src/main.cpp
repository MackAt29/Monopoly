...
int main(int argc, char** argv) {
    ...
    std::cout<<"Turn order: ";
    for (int i = 0; i < 4; ++i) {
        std::cout<<"P"<<std::string(1, rounds[i].getName())<<" ";
    }
    std::cout<<std::endl;
            ...
            if(rounds[i].getPosition()%7!=0){
                ...
                if (casella.getOwner() != nullptr && casella.getOwner() != &rounds[i]) {
                    ...
                    if(rounds[i].getGold()<=0){
                        std::cout << "Player " << std::string(1, rounds[i].getName()) << " has been terminated" << std::endl;
                ...
                //STEP 7.2: E' una casella vuota o è la tua casella??? In ogni caso puoi investire su questa casella
                    ...
                    if(casella.getOwner() == nullptr){
                        ...
                        if(rounds[i].getGold()<casella.getBuyPrice()){
                            std::cout << "Player " << std::string(1, rounds[i].getName()) << " has not enough money to buy Property nr. "<< rounds[i].getPosition()+1 << std::endl;
                            rounds[i].logToFile("Player " + std::string(1, rounds[i].getName()) + " has not enough money to buy Property nr. " + std::to_string(rounds[i].getPosition()+1));
                        }
                        ...
                        if(rounds[i].isHumanPlayer()){
                            ...
                            if(choice){
                                ...
                                if(casella.getStatus()==1){
                                    ...
                                    casella.buyHouse();
                                    std::cout << "Player " << std::string(1, rounds[i].getName()) << " has built an house on Property nr. "<< rounds[i].getPosition()+1 << std::endl;
                                    rounds[i].logToFile("Player " + std::string(1, rounds[i].getName()) + " has built an house on Property nr. " + std::to_string(rounds[i].getPosition()+1));
                                } 
                                //SE questa casella ha una casa, potenziala in un albergo
                                else if(casella.getStatus()==2){
                                    rounds[i].payProperty(casella.getHotelPrice());
                                    casella.buyHotel();
                                    std::cout << "Player " << std::string(1, rounds[i].getName()) << " has upgraded the Property nr. "<< rounds[i].getPosition()+1<< " into an hotel." << std::endl;
                                    rounds[i].logToFile("Player " + std::string(1, rounds[i].getName()) + " has upgraded the Property nr. " + std::to_string(rounds[i].getPosition()+1) + " into an hotel.");
                                } 
                            }
                        }
                    }
    ...
    if(rounds.size()==1) {
        std::cout << "The WINNER is: P" << std::string(1, rounds[0].getName()) << std::endl;
        rounds[0].logToFile("The WINNER is: P" + std::string(1, rounds[0].getName()));
    }   
    //STEP 9.2: Se la partita era tra 4 CPU E sono passati 10 turni stampa il giocatore che aveva più fiorini alla fine della partita
        ...
        std::cout << "The WINNER/s for the greatest amount of florins is/are: P" << std::string(1, rounds[c].getName());
        rounds[c].logToFile("The WINNER for the greatest amount of florins is: P" + std::string(1, rounds[c].getName()));
        //"In caso di quantità uguale di fiorini, è permessa la vittoria ex-equo"
        for(int i = 0; i<rounds.size();++i){
            if(rounds[c].getGold()==rounds[i].getGold()&& c!=i){
                std::cout << " P" << std::string(1, rounds[c].getName());
            }
        }
    }
    return 0;
